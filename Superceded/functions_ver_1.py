...
def plot_reccurence(
    ...
    if stride == 0:  # if using non overlapping splits
        ...
        array_list = []
        for sensor in range(input_array.shape[0]):
            arr = input_array[sensor]  # iterate through rows (sensors) in array
            e = window(arr, length, stride, copy=True)  # generate sliding window arrays
            array_list.append(e)  # append to array_list
    ...
    if stride == 0:
        ...
        for i, a in enumerate(array_list):
            # for each array in arraylist (corresponds to a sensor)
            for j, _ in enumerate(a):
                # for each array row (corresponds to sliding window for sensor)
                # here we are stepping through 1 sensor at a time and generating plots for each sliding window
                ...
                # filepath
                path = Path(
                    f"../RP/{direct}/{strided}/distance_{distance}/length_{length}/stride_{stride}"
                ).mkdir(parents=True, exist_ok=True)
                ...
                ImageGenerator.save_recurrence_plot(
                    result.recurrence_matrix_reverse,
                    f"../RP/{direct}/{strided}/distance_{distance}/length_{length}/stride_{stride}/Iteration_{iteration}_Sensor_{i}_Split_{j}_{target}.png",
                )
                ...
                # save rp filenames with time window, sensor name, and label (in this case, 0 = normal)
                with open(
                    f"../RP/{direct}/{strided}/distance_{distance}/length_{length}/stride_{stride}/mapping.csv",
                    "a",
                ) as fileobj:
                    fileobj.write(
                        f"{data_idx},Iteration_{iteration}_Sensor_{i}_Split_{j}_{target}.png,{start_date},{end_date},{label}\n"
                    )
                    data_idx += 1
